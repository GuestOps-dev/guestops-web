...
- [ ] **Prove the native WhatsApp group workflow with consenting test participants.** Before further group-dependent product work, confirm the provider-supported path can create a separate native WhatsApp group containing the GuestOps business sender plus Scott and one consenting tester; verify every participant can see messages, the inbound event reaches GuestOpsHQ, and an approved app reply appears in that same group. Do not use a guest, vendor, or any real reservation contact. Twilio's relay-style Conversations sample is not sufficient evidence of native group support.
