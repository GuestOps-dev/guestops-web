...
- [ ] **Add property maintenance intelligence.** When AI identifies a likely guest-reported issue (for example AC trouble, a damaged mirror, or a jammed lock), create an internal maintenance issue tied to the property and source conversation. Give the team a separate maintenance list with priority, open/in-progress/resolved status, assignee, evidence, and recurrence history. This must remain a recommendation/review workflow initially: it never sends a guest or vendor message automatically.
...
- Vendor coordination release prepared: an experience will prepare a private team-and-vendor-only coordination record and draft with party size, date/time, pickup details, and notes. It never includes the guest or creates a live WhatsApp group/message; publish after the pending database migration is applied.
