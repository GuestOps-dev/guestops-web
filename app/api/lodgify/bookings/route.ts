...
export async function GET(req: Request) {
  ...
  try {
    ...
    const bookings = (await fetchBookings()).map((booking) => {
      ...
      return { id, property_id: property.property_id, property_name: property.property_name, guest_name: text(booking.guest?.name) ?? text(booking.guest?.guest_name), arrival: date(booking.arrival), departure: date(booking.departure), booked_at: timestamp(booking.created_at) ?? timestamp(booking.booked_at) ?? timestamp(booking.booking_date), status: text(booking.status), source: "Lodgify", is_new: booking.is_new === true, party_size: partySize(booking), whatsapp_group_prepared: property.whatsapp_group_default_enabled };
    }).filter(Boolean);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (property.whatsapp_group_default_enabled) {
      try {
        await prepareGroupForStay({ propertyId: property.property_id, bookingId: booking.id, conversationId: conversation.id, guestId, guestName: fullName, guestPhone: phone });
      } catch (groupError) {
