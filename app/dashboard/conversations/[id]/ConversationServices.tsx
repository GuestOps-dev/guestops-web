...
export default function ConversationServices({ propertyId, bookingId }: { propertyId: string; bookingId?: string | null }) {
  ...
  async function createService() {
    ...
    try {
      const response = await fetch("/api/experiences", {
        ...
        body: JSON.stringify({ property_id: propertyId, booking_id: bookingId, experience_type_id: serviceTypeId, vendor_id: vendorId || null, start_at: startsAt || null, internal_notes_private: notes || null }),
      });
      const data = await response.json().catch(() => null);
      if (!response.ok) throw new Error(data?.error ?? "Unable to plan experience");
      setItems((current) => [data, ...current]); setStartsAt(""); setNotes(""); setVendorId("");
  ...
  return <section style={{ marginTop: 18, borderTop: "1px solid #e2e8f0", paddingTop: 14 }}>
    ...
    {items.length ? <div style={{ display: "grid", gap: 7, marginTop: 10 }}>{items.map((item) => <div key={item.id} style={{ padding: "7px 8px", borderRadius: 7, border: "1px solid #dbe4ee", background: "#fff", fontSize: 12 }}><strong>{item.experience_types?.name ?? "Experience"}</strong><span style={{ color: "#64748b" }}> · {item.status.replaceAll("_", " ")}</span>{item.vendors?.name ? <div style={{ marginTop: 2 }}>Vendor: {item.vendors.name}</div> : <><div style={{ marginTop: 2, color: "#64748b" }}>Vendor not chosen yet</div>{item.status === "proposed" ? <div style={{ display: "flex", gap: 5, marginTop: 6 }}><select value={vendorChoices[item.id] ?? ""} onChange={(event) => setVendorChoices((current) => ({ ...current, [item.id]: event.target.value }))} style={{ ...inputStyle, flex: 1, padding: "5px 6px" }}><option value="">Choose vendor…</option>{vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name}</option>)}</select><button type="button" disabled={busy || !vendorChoices[item.id]} onClick={() => void assignVendor(item)} style={smallButtonStyle}>Assign</button></div> : null}</>}{item.start_at ? <div style={{ color: "#64748b", marginTop: 2 }}>{new Date(item.start_at).toLocaleString()}</div> : null}{item.internal_notes_private ? <div style={{ color: "#475569", marginTop: 4, whiteSpace: "pre-wrap" }}>{item.internal_notes_private}</div> : null}{item.status === "proposed" ? <div style={{ display: "flex", gap: 6, marginTop: 7 }}><button type="button" disabled={busy || !item.vendors} onClick={() => void updateService(item, "contacted")} style={smallButtonStyle}>Mark vendor contacted</button><button type="button" disabled={busy} onClick={() => void updateService(item, "cancelled")} style={quietButtonStyle}>Cancel</button></div> : null}{item.status === "vendor_contacted" ? <div style={{ display: "flex", gap: 6, marginTop: 7 }}><button type="button" disabled={busy} onClick={() => void updateService(item, "confirmed")} style={smallButtonStyle}>Mark confirmed</button><button type="button" disabled={busy} onClick={() => void updateService(item, "cancelled")} style={quietButtonStyle}>Cancel</button></div> : null}</div>)}</div> : null}
  </section>;
}
