...
export default function ConversationServices({ propertyId, bookingId }: { propertyId: string; bookingId?: string | null }) {
  ...
  async function load() {
    ...
    try {
      ...
      if (!typeResponse.ok || !vendorResponse.ok || !experienceResponse.ok) throw new Error("Unable to load concierge services");
      setTypes(await typeResponse.json()); setVendors(await vendorResponse.json()); setItems(await experienceResponse.json());
    } catch (e: any) { setError(e?.message ?? "Unable to load concierge services"); }
  }
  ...
  async function addType() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.error ?? "Unable to add service type");
      setTypes((current) => [...current, data].sort((a, b) => a.name.localeCompare(b.name)));
      setServiceTypeId(data.id); setNewType("");
    } catch (e: any) { setError(e?.message ?? "Unable to add service type"); } finally { setBusy(false); }
  }
  ...
  async function createService() {
    ...
    try {
      ...
      const data = await response.json().catch(() => null);
      if (!response.ok) throw new Error(data?.error ?? "Unable to create service request");
  ...
  return <section style={{ marginTop: 18, borderTop: "1px solid #e2e8f0", paddingTop: 14 }}>
    ...
    {types.length === 0 ? <div style={{ padding: 9, borderRadius: 8, background: "#f8fafc", border: "1px solid #e2e8f0", fontSize: 12 }}>
      ...
      <button type="button" disabled={busy || !serviceTypeId} onClick={() => void createService()} style={{ ...buttonStyle, justifySelf: "start" }}>{busy ? "Saving…" : "Plan service"}</button>
