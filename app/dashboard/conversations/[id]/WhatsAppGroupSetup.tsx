...
export default function WhatsAppGroupSetup({ group }: { group: WhatsAppGroupSetupData | null }) {
  ...
  const label = active ? "Active" : failed ? "Needs attention" : group.status === "closed" ? "Closed" : "Ready to set up";
  ...
  return <section style={{ marginBottom: 10, padding: "10px 12px", border: `1px solid ${border}`, borderRadius: 10, background, fontSize: 12 }}>
    <div style={{ display: "flex", gap: 8, justifyContent: "space-between", flexWrap: "wrap", alignItems: "baseline" }}>
      <strong style={{ color: "#0f172a" }}>WhatsApp group · {group.display_name}</strong>
      <span style={{ color, fontWeight: 700 }}>{label}</span>
    </div>
    {active ? <p style={{ margin: "5px 0 0", color: "#475569" }}>This group is active through the supported provider connection.</p> : <p style={{ margin: "5px 0 0", color: "#475569" }}>The participant list is prepared below. No group or guest message has been created automatically.</p>}
